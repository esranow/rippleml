...
def test_laplacian_block_forward():
    block = rnn.HybridLaplacianBlock(mode="point", correction_input_dim=2)
...
def test_spectral_block_forward():
    block = rnn.SpectralHybridBlock(n_modes=64, cutoff=8)
    u = torch.randn(2, 64)
    out = block(u)
    assert out.shape == (2, 64)

def test_energy_block_forward():
    block = rnn.EnergyAwareBlock(c=1.0, spatial_dim=1)
...
def test_oscillator_block_step():
    block = rnn.HybridOscillatorBlock(omega=1.0, alpha=0.0)
    state = torch.randn(4, 2)
    next_state = block.step(state, dt=0.01)
    assert next_state.shape == (4, 2)

def test_embedding_block_forward():
    block = rnn.PDEParameterEmbeddingBlock(param_dim=3, embed_dim=16, feature_dim=32)
    params = torch.randn(4, 3)
    emb = block(params=params)
    assert emb.shape == (4, 16)

def test_block_pipeline():
    lap_block = rnn.HybridLaplacianBlock(mode="grid", spatial_dim=1, use_correction=False)
    spec_block = rnn.SpectralHybridBlock(n_modes=64, cutoff=16, use_correction=False)
